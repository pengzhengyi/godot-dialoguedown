...
public sealed class CompileCommandTests
{
    ...
    [Fact]
    public void Compile_MissingFile_FailsValidation()
    {
        var tester = CliTester.Create();
    ...
    [Fact]
    public void Compile_WrongExtension_FailsValidation()
    {
        var tester = CliTester.Create();

        var result = tester.Run("compile", "notes.txt");

        Assert.NotEqual(ExitCodes.Success, result.ExitCode);
        Assert.Contains(".dialogue.md", result.Output, StringComparison.Ordinal);
    }
}
