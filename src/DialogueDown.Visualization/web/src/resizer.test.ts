...
describe("initResizer", () => {
    ...
    it("resizes the detail panel while dragging (width = innerWidth − clientX)", () => {
        ...
        expect(detail.style.getPropertyValue("--detail-size")).toBe("500px");
    });

    it("clamps the width to the [280, 760] range", () => {
        resizer.dispatchEvent(new MouseEvent("mousedown"));
        drag(window.innerWidth - 5000);
        expect(detail.style.getPropertyValue("--detail-size")).toBe("760px");
        drag(window.innerWidth - 10);
        expect(detail.style.getPropertyValue("--detail-size")).toBe("280px");
    });
    ...
    it("measures the detail's right edge so content padding doesn't offset the divider", () => {
        ...
        resizer.dispatchEvent(new MouseEvent("mousedown"));
        drag(600);
        expect(detail.style.getPropertyValue("--detail-size")).toBe("400px"); // 1000 − 600, not innerWidth − 600
    });

    it("does nothing until a drag starts and after it ends", () => {
        drag(window.innerWidth - 400);
        expect(detail.style.getPropertyValue("--detail-size")).toBe("");

        resizer.dispatchEvent(new MouseEvent("mousedown"));
        drag(window.innerWidth - 400);
        expect(detail.style.getPropertyValue("--detail-size")).toBe("400px");
    ...
    it("does not resize while the layout is stacked (a column app)", () => {
        // On narrow screens the divider is a collapse handle only, so a width drag is ignored.
        document.body.innerHTML = `<div id="app" style="flex-direction: column"><div id="resizer"></div><aside id="detail"></aside></div>`;
        resizer = document.getElementById("resizer")!;
        detail = document.getElementById("detail")!;
        initResizer();
        resizer.dispatchEvent(new MouseEvent("mousedown"));
